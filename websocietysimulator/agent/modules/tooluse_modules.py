...
class ToolUseIO(ToolUseBase):
    ...
    def __call__(self, task_description, tool_instruction, feedback_of_previous_tools):
        tool_pool = tooluse_pool.get(task_description)
...
'''
        ...
        messages = [{"role": "user", "content": prompt}]
        return self.llm(messages=messages, temperature=0.1)
...
class ToolUseToolBenchFormer(ToolUseBase):
    ...
    def __call__(self, task_description, tool_instruction, feedback_of_previous_tools):
        ...
        messages = [{"role": "user", "content": prompt}]
        strings = self.llm(messages=messages, temperature=0.1, n=3)
...
'''     
        for i, y in enumerate(strings, 1):
            prompt += f'Answer {i}:\n{y}\n'
        messages = [{"role": "user", "content": prompt}]
        vote_outputs = self.llm(messages=messages, temperature=0.3, n=5)
